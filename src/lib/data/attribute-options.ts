...
export async function listOptions(
  ...
  const [attr] = await db.select().from(product_attributes).where(eq(product_attributes.id, attributeId));
  ...
  const transRows = await db.select().from(translations).where(inArray(translations.entity_id, optionIds));
  const transMap = new Map(
    transRows.filter(t => t.entity_type === 'product_attribute_option' && t.locale === locale).map(t => [t.entity_id, t]),
  );
...
export async function getOption(
  ...
  return { id: opt.id, attribute_id: opt.attribute_id, value: opt.value, label, sort_order: opt.sort_order };
}
...
export async function createOption(
  ...
  const [attr] = await db.select().from(product_attributes).where(eq(product_attributes.id, attributeId));
  if (!attr) throw new OptionError('Attribute not found', 'not_found');
  if (attr.type !== 'select') throw new OptionError('Options can only be added to select-type attributes', 'not_select');

  const id = crypto.randomUUID();
  await db.insert(product_attribute_options).values({ id, attribute_id: attributeId, value: input.value, sort_order: input.sort_order });
...
export async function updateOption(
  ...
  const [existing] = await db.select().from(product_attribute_options).where(eq(product_attribute_options.id, optionId));
  ...
  if (Object.keys(updateData).length > 0) {
    await db.update(product_attribute_options).set(updateData).where(eq(product_attribute_options.id, optionId));
  }
  return { id: optionId, value: input.value ?? existing.value, sort_order: input.sort_order ?? existing.sort_order };
}
...
export async function deleteOption(
  ...
  if (hasUsage) throw new OptionError('Option is used in product assignments. Remove from products first.', 'in_use');
