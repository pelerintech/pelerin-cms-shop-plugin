...
export async function migrateDefaultLocale(
  ...
  return db.transaction(async (tx) => {
    ...
    for (const product of allProducts) {
      ...
      const [newDefaultTranslation] = await tx.select().from(translations).where(
        and(
      ...
      const [existingOldTranslation] = await tx.select().from(translations).where(
        and(
          eq(translations.entity_type, 'product'),
          eq(translations.entity_id, product.id),
      ...
      const alreadyMigrated = newDefaultTranslation
      ...
      if (!alreadyMigrated) {
        if (existingOldTranslation) {
          await tx.update(translations).set({
            name: product.name,
    ...
    for (const category of allCategories) {
      ...
      const [existingOldCatTranslation] = await tx.select().from(translations).where(
        and(
          eq(translations.entity_type, 'category'),
          eq(translations.entity_id, category.id),
          eq(translations.locale, oldLocale),
      ...
      const alreadyMigratedCat = newDefaultTranslation
    ...
    for (const attribute of allAttributes) {
      ...
      const [existingOldAttrTranslation] = await tx.select().from(translations).where(
        and(
          eq(translations.entity_type, 'product_attribute'),
          eq(translations.entity_id, attribute.id),
          eq(translations.locale, oldLocale),
      ...
      const alreadyMigratedAttr = newDefaultTranslation
        && attribute.name === (newDefaultTranslation.name ?? attribute.name);
      ...
      if (newDefaultTranslation) {
        await tx.update(product_attributes).set({
    ...
    for (const option of allOptions) {
      ...
      const [existingOldOptionTranslation] = await tx.select().from(translations).where(
        and(
          eq(translations.entity_type, 'product_attribute_option'),
          eq(translations.entity_id, option.id),
          eq(translations.locale, oldLocale),
