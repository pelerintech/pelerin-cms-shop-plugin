...
import { getReferralById, updateReferral, deleteReferral, ReferralError } from '../../../lib/data/referrals';
...
export async function runGet({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    if (!r) return new Response(JSON.stringify({ success: false, error: 'Referral not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
    return new Response(JSON.stringify({ success: true, data: r }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (err: any) {
...
export async function runPut({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    if (err instanceof ReferralError) return new Response(JSON.stringify({ success: false, error: err.message }), { status: 404, headers: { 'Content-Type': 'application/json' } });
    return new Response(JSON.stringify({ success: false, error: err.message || 'Server Error' }), { status: err.status ?? 500, headers: { 'Content-Type': 'application/json' } });
  }
}
