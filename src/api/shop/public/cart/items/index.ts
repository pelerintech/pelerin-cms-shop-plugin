...
export async function runPost({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    try {
      const item = await addCartItem(db, cart.id, { product_id, variant_id: variant_id || null, quantity });
      const headers: Record<string, string> = { 'Content-Type': 'application/json' };
      if (setCookie) headers['Set-Cookie'] = setCookie;
      return new Response(JSON.stringify({
        success: true, data: { cart_id: cart.id, product_id, variant_id: variant_id || null, quantity: item.quantity },
      }), { status: 200, headers });
    } catch (e: any) {
      if (e instanceof CartItemError) {
        const codeMap: Record<string, number> = {
          not_found: 404, product_not_found: 404, out_of_stock: 409,
          insufficient_stock: 409, variant_required: 422,
        };
        return new Response(JSON.stringify({ success: false, error: e.message }), {
          status: codeMap[e.code] ?? 400, headers: { 'Content-Type': 'application/json' },
        });
      }
      throw e;
    }
