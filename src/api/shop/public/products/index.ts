...
import { listProducts, getProductWithPrices, resolveProductBySlug, resolveCategoryBySlug, SlugCollisionError } from '../../../../lib/data/products';
...
async function enrichProduct(db: any, p: any, locale: string, currency: string): Promise<any | null> {
  ...
  return {
    id: p.id, sku: p.sku, name: withPrices.name, description: withPrices.description, slug: withPrices.slug, type: p.type,
...
export async function runGet({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    if (slug) {
      ...
      if (result === null) {
        return new Response(JSON.stringify({ success: false, error: 'Product not found' }), { status: 404, headers: { 'Content-Type': 'application/json' } });
      }
      if (result instanceof SlugCollisionError) {
        return new Response(JSON.stringify({ success: false, error: `Slug collision: ${result.message}` }), { status: 409, headers: { 'Content-Type': 'application/json' } });
      }
    ...
    if (categorySlug && !categoryId) {
      ...
      if (catResult instanceof SlugCollisionError) {
        return new Response(JSON.stringify({ success: false, error: `Slug collision: ${catResult.message}` }), { status: 409, headers: { 'Content-Type': 'application/json' } });
      }
      resolvedCategoryId = catResult.category.id;
    }

    // List mode.
    const result = await listProducts(db, { page: 1, limit: 100, locale, category_id: resolvedCategoryId, active: true });
    ...
    if (err instanceof SlugCollisionError) {
      return new Response(JSON.stringify({ success: false, error: `Slug collision: ${err.message}` }), { status: 409, headers: { 'Content-Type': 'application/json' } });
    }
