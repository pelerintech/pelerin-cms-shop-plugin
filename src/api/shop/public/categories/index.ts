...
import { listCategories, resolveCategoryBySlug, SlugCollisionError } from '../../../../lib/data/products';
...
export async function runGet({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    if (slug) {
      ...
      return new Response(JSON.stringify({ success: true, data: result.category }), { status: 200, headers: { 'Content-Type': 'application/json' } });
    }
