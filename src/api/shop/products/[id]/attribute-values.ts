...
import { db, products, product_attribute_values, product_attribute_assignments, product_attributes, product_attribute_options, translations, sql as dbSql } from 'astro:db';
...
export const GET: APIRoute = async (context) => {
  ...
  try {
    ...
    const assignments = await db
      ...
      .where(
        dbSql`${product_attribute_assignments.product_id} = ${productId} AND ${product_attribute_assignments.role} = 'field'`
      )
      .orderBy(product_attribute_assignments.sort_order);

    // Fetch product-level attribute values
    const assignmentIds = assignments.map(a => a.id);
    let values: any[] = [];
    if (assignmentIds.length > 0) {
      const valResult = await db.run(
        dbSql`SELECT * FROM ${product_attribute_values} WHERE ${product_attribute_values.entity_type} = 'product' AND ${product_attribute_values.entity_id} = ${productId} AND ${product_attribute_values.assignment_id} IN (${dbSql.join(assignmentIds.map(id => dbSql`${id}`))})`
      );
  ...
  try {
    ...
    for (const val of body.values) {
      ...
      const [assignment] = await db
        ...
        .from(product_attribute_assignments)
        .where(
          dbSql`${product_attribute_assignments.id} = ${assignment_id} AND ${product_attribute_assignments.product_id} = ${productId} AND ${product_attribute_assignments.role} = 'field'`
        );
      ...
      if (!assignment) {
        return new Response(
          JSON.stringify({
            success: false,
            error: `Assignment ${assignment_id} does not belong to this product or is not a field role`,
          }),
          { status: 422, headers: { 'Content-Type': 'application/json' } }
        );
      }
      ...
      const [existingValue] = await db
        ...
        .where(
          dbSql`${product_attribute_values.entity_type} = 'product' AND ${product_attribute_values.entity_id} = ${productId} AND ${product_attribute_values.assignment_id} = ${assignment_id}`
        );
