...
export async function runGet({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    return new Response(JSON.stringify({ success: true, data: result.products, total: result.total, page: result.page, limit: result.limit }), {
...
export async function runPost({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    return new Response(JSON.stringify({ success: true, data: { id, ...parsed.data } }), { status: 201, headers: { 'Content-Type': 'application/json' } });
  } catch (err: any) {
    const status = err.status ?? 500;
    return new Response(JSON.stringify({ success: false, error: err.message || 'Server Error' }), { status, headers: { 'Content-Type': 'application/json' } });
  }
