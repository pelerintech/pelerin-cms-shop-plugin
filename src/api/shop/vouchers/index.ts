...
export async function runGet({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    return new Response(JSON.stringify({ success: true, data: vouchers }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  } catch (err: any) {
...
export async function runPost({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    return new Response(JSON.stringify({ success: true, data: voucher }), { status: 201, headers: { 'Content-Type': 'application/json' } });
  } catch (err: any) {
    return new Response(JSON.stringify({ success: false, error: err.message || 'Server Error' }), { status: err.status ?? 500, headers: { 'Content-Type': 'application/json' } });
  }
}
