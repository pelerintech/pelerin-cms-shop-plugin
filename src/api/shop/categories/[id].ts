...
export async function runGet({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    return new Response(JSON.stringify({ success: true, data: cat }), {
      status: 200,
      headers: { 'Content-Type': 'application/json' },
    });
  } catch (err: unknown) {
...
export async function runPut({ db, sdk, ctx }: HandlerDeps): Promise<Response> {
  try {
    ...
    return new Response(
      JSON.stringify({ success: true, data: { id: ctx.params.id, ...parsed.data } }),
    ...
    if (err instanceof SlugCollisionError) {
      return new Response(
        JSON.stringify({
          success: false,
          error: 'Validation failed',
          fields: { [`slug_${err.locale}`]: errorFields(err).message },
        }),
        { status: 422, headers: { 'Content-Type': 'application/json' } }
      );
    }
