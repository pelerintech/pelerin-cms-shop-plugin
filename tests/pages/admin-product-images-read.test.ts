...
test('admin product edit page calls listProductImage(db, sdk, id) and passes resolved rows to ImageUpload', () => {
  assert.match(
    source,
    /import\s+\{[^}]*\blistProductImage\b[^}]*\}\s+from\s+['"][^'"]*lib\/data\/products['"]/,
    'page must import listProductImage'
  );
  assert.match(
    source,
    /listProductImage\(\s*sdk\.db\s*,\s*sdk\s*,/,
    'page must call listProductImage(sdk.db, sdk, id) — db from sdk'
  );
...
test('admin product edit page constructs an sdk via createPluginContext() and passes it to the accessor', () => {
  assert.match(
    source,
    /createPluginContext\(\)/,
    'page must construct sdk via createPluginContext()'
  );
});
