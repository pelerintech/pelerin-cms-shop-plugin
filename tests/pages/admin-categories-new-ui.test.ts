...
test('category new page renders non-default locale sections', () => {
  assert.ok(source.includes('otherLocales'), 'Page should define otherLocales variable');
  assert.ok(
...
test('category new page has default locale section label', () => {
  assert.ok(source.includes('(default)'), 'Page should label the default locale section');
});
...
test('category new page shows warning when no locales configured', () => {
  ...
  assert.ok(source.includes('hasLocales'), 'Page should check hasLocales before rendering warning');
});
...
test('category new page uses SearchSelect for parent category', () => {
  ...
  assert.ok(source.includes('name="parent_id"'), 'SearchSelect should have name="parent_id"');
  assert.ok(
