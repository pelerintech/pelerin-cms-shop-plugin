/**
 * Client script syntax guard for the voucher [id] admin page (r40 touched its
...
const PAGE_PATH = join(__dirname, '../../src/pages/admin/vouchers/[id].astro');
const ESBUILD = join(__dirname, '../../node_modules/esbuild/bin/esbuild');

describe('vouchers [id].astro client <script> syntax', () => {
  it('transforms with esbuild without a syntax/duplicate-declaration error', () => {
    ...
    assert.equal(
      exitCode,
      0,
      `client <script> in vouchers/[id].astro has a syntax error.\nesbuild output:\n${combined}`
    );
  });
});
