...
const LIST_PAGE = readFileSync(
  new URL('../../src/pages/admin/orders/index.astro', import.meta.url),
  'utf-8'
);
const DETAIL_PAGE = readFileSync(
  new URL('../../src/pages/admin/orders/[id].astro', import.meta.url),
  'utf-8'
);
...
test('orders list page: status badge color map includes partially_refunded', () => {
  assert.match(
    LIST_PAGE,
    /partially_refunded:\s*['"]badge-[^'"]+['"]/,
    'statusBadge colors map must include partially_refunded'
  );
});
...
test('orders list page: status label map includes partially_refunded', () => {
  assert.match(
    LIST_PAGE,
    /partially_refunded:\s*['"][^'"]+['"]/,
    'statusLabel labels map must include partially_refunded'
  );
});
...
test('orders detail page: status badge color map includes partially_refunded', () => {
  assert.match(
    DETAIL_PAGE,
    /partially_refunded/,
    'detail page badge map must reference partially_refunded'
  );
});
