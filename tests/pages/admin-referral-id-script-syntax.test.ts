/**
 * Client script syntax guard for the referral [id] admin page (r40 touched its
 * client script for the money-unit display conversion).
 */
...
const PAGE_PATH = join(__dirname, '../../src/pages/admin/referral-codes/[id].astro');
const ESBUILD = join(__dirname, '../../node_modules/esbuild/bin/esbuild');

describe('referral [id].astro client <script> syntax', () => {
  it('transforms with esbuild without a syntax/duplicate-declaration error', () => {
    ...
    assert.equal(
      exitCode,
      0,
      `client <script> in referral-codes/[id].astro has a syntax error.\nesbuild output:\n${combined}`
    );
  });
});
