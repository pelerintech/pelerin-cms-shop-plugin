...
test('webhook_url is built from the public base, not the request origin', async () => {
  const { db, cleanup } = await createTestDb();
  try {
    const f = await seedMinimal(db);
    ...
    // Public base differs from the request origin, mirroring a TLS-terminating
    // proxy where the node adapter derives the origin as http:// from the socket.
    process.env.BETTER_AUTH_URL = 'https://cms.geneticlab.ro';
    const sdk = makeFakeSdk({ user: null });
    const ctx = makeCtx({
      url: 'http://internal:3000/api/plugins/shop/public/checkout/order-1/pay',
      method: 'POST',
      body: {
        success_url: 'https://shop.example.com/success',
        cancel_url: 'https://shop.example.com/cart',
    ...
    const redirectUrlStr = b.data.redirect_url;
    // The silenturl (webhook_url) must be the public base, not the internal origin.
    assert.ok(
      redirectUrlStr.includes(
        'https%3A%2F%2Fcms.geneticlab.ro%2Fapi%2Fplugins%2Fshop%2Fwebhooks%2Feuplatesc'
      ),
      'redirect should contain the encoded public webhook URL'
    );
    assert.ok(
