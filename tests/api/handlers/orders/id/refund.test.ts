...
async function seedOrder(db: any, f: any, orderNumber = 'ORD-R', cartId = 'cart-refund') {
...
test('PUT validation-fail → 422', () =>
  matrix.validationFail({
    ...
    url: base + 'x',
    params: { id: 'x' },
    invalidBody: { refund_amount: 'not-a-number' },
  }));

test('PUT happy-path → 200, status refunded', async () => {
  const { db, cleanup } = await createTestDb();
  try {
    const f = await seedMinimal(db);
    const order = await seedOrder(db, f, 'ORD-RH', 'cart-refund-h');
    // Move to refund_requested so refund→refunded transition is valid
    ...
    const ctx = makeCtx({
      url: base + order.id,
      body: { refund_amount: 1000, refund_notes: 'partial refund' },
      params: { id: order.id },
      method: 'PUT',
    });
    ...
    assert.equal(b.data.status, 'refunded');
  } finally {
    await cleanup();
  }
});
...
test('PUT 404: unknown id → 404', async () => {
  ...
  try {
    ...
    const ctx = makeCtx({
      url: base + 'nope',
      body: { refund_amount: 100, refund_notes: null },
      params: { id: 'nope' },
      method: 'PUT',
    });
    const res = await runPut({ db, sdk, ctx });
...
test('PUT 422: refund_amount exceeds total → 422', async () => {
  const { db, cleanup } = await createTestDb();
  try {
    const f = await seedMinimal(db);
    const order = await seedOrder(db, f, 'ORD-R422', 'cart-refund-422');
    await transitionOrderStatus(db, order.id, 'awaiting_payment');
    await transitionOrderStatus(db, order.id, 'paid');
    await transitionOrderStatus(db, order.id, 'refund_requested');
    const sdk = makeFakeSdk();
    const ctx = makeCtx({
      url: base + order.id,
      // order total is 5250; refund 99999 exceeds it
      body: { refund_amount: 99999, refund_notes: null },
      params: { id: order.id },
      method: 'PUT',
    });
    const res = await runPut({ db, sdk, ctx });
