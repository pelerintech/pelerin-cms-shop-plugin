...
ensureLoader();
const { runPut, runDelete } = await import('../../../../../../src/api/shop/products/[id]/variants/[variantId].ts');
...
test('PUT happy-path → 200, data.id matches', async () => {
  ...
  try {
    ...
    const ctx = makeCtx({ url: base, body, params: { id: f.variantProductId, variantId: f.variantBlack128Id } });
...
test('PUT error-wrap → 500', () =>
  matrix.errorWrap({ run: runPut, url: base, body: { sku: 'x' }, params: { id: 'p', variantId: 'v' } }));
...
test('DELETE happy-path → 200, data null', async () => {
  ...
  try {
    const f = await seedMinimal(db);
    const sdk = makeFakeSdk();
    const ctx = makeCtx({ url: base, params: { id: f.variantProductId, variantId: f.variantBlack128Id } });
...
test('PUT accepts prices: upserts RON override and deletes EUR (price_net:null)', async () => {
  ...
  try {
    ...
    const rows = await db.select().from(product_prices).where(eq(product_prices.variant_id, f.variantBlack128Id));
