...
ensureLoader();
const { UploadProductImageSchema, ProductImageOutputSchema } = await import(
...
test('UploadProductImageSchema has NO url field (url comes from storage, not user input)', () => {
  ...
  assert.ok(!('url' in parsed.data) || parsed.data.url === undefined, 'url must not be a user-facing field');
});

test('ProductImageOutputSchema still has url (now a RESOLVED url in outputs)', () => {
  const out = ProductImageOutputSchema.safeParse({
    id: 'i1', product_id: 'p1', variant_id: null, url: '/uploads/products/p1/x.png',
    alt: null, sort_order: 0,
  });
...
test('the upload handler source no longer references CreateProductImageSchema or JSON url body', () => {
  const src = readFileSync(new URL('../../src/api/shop/products/[id]/images/index.ts', import.meta.url), 'utf-8');
  assert.doesNotMatch(src, /CreateProductImageSchema/, 'handler must not reference the old JSON-url schema');
