...
describe('Stripe refund stub', () => {
  it('returns not-implemented error', async () => {
    ...
    const result = await provider.refund(null as any, { transaction_id: null } as any, 5000, 'test reason');

    assert.strictEqual(result.success, false, 'refund must return success: false');
    assert.ok(result.error?.includes('not yet implemented') || result.error?.includes('Not implemented'),
