...
describe('Stripe adapter — initiate payment', () => {
  ...
  it('exports initiatePayment function', () => {
    ...
    assert.match(
      content,
      /export.*initiatePayment|async function initiatePayment/,
  ...
  it('exports handleWebhook function', () => {
    ...
    assert.match(
      content,
      /export.*handleWebhook|async function handleWebhook/,
      'Should have handleWebhook'
    );
  });
  ...
  it('creates Stripe checkout session', () => {
    ...
    assert.match(
      content,
      /checkout\.sessions\.create|sessions\.create/,
      'Should call Stripe checkout sessions create'
    );
  });
  ...
  it('loads credentials via getSetting accessor', () => {
    ...
    assert.match(
      ...
      'Should use getSetting accessor for stripe_secret_key'
    );
  });
  ...
  it('sets client_reference_id to order.id', () => {
    ...
    assert.match(
      ...
      'Should set client_reference_id to order.id'
    );
  });
