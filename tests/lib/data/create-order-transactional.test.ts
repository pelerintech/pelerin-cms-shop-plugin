...
test('(happy) createOrder: 2 items, sufficient stock → order + items + history + stock decrement + cart cleared', async () => {
  ...
  try {
    ...
    const order = await createOrder(
      db,
      baseOrderInput('ORD-HAPPY', cartId, [
        {
...
test('(rollback) a forced failure mid-flow rolls back order, items, history, stock, cart', async () => {
  ...
  try {
    ...
    const first = await createOrder(
      db,
      baseOrderInput('ORD-COLLIDE', 'cart-rollback-first', [
        {
          product_id: f.simpleProductId,
          variant_id: null,
          product_name: 'Carte',
    ...
    await db.insert(orders).values({
      ...
      billing_postal_code: '1',
      billing_country: 'RO',
      shipping_first_name: 'X',
      shipping_last_name: 'Y',
      shipping_address: 'A',
      shipping_city: 'C',
      shipping_postal_code: '1',
      shipping_country: 'RO',
    ...
    await db
      ...
      .set({ value: '1' })
      .where(eq(shop_settings.key, 'order_number_sequence'));
    ...
    const secondOrder = await createOrder(
      db,
      baseOrderInput('ORD-SHOULD-RETRY', cartId, [
        {
          product_id: f.simpleProductId,
          variant_id: null,
          product_name: 'Carte',
    ...
    assert.notEqual(
      ...
      collideNum,
      'retry must produce a non-colliding number'
    );
  ...
  try {
    ...
    await assert.rejects(
      () =>
        createOrder(
          db,
          baseOrderInput('ORD-INSUFF', cartId, [
            {
              product_id: f.simpleProductId,
              variant_id: null,
              product_name: 'Carte',
...
test('(rollback) a mid-transaction throw leaves NO partial order, items, history, or stock change', async () => {
  ...
  try {
    ...
    for (const seq of [2, 3, 4]) {
      await db.insert(orders).values({
        id: `pre-${seq}`,
        order_number: `ORD-${year}-${String(seq).padStart(6, '0')}`,
        user_id: null,
        customer_type: 'individual',
        customer_email: 'x@e.com',
        customer_name: 'X',
    ...
    await assert.rejects(() =>
      createOrder(
        db,
        baseOrderInput('ORD-THROW', cartId, [
          {
            product_id: f.simpleProductId,
            variant_id: null,
            product_name: 'Carte',
    ...
    assert.equal(
      allOrders.length,
      3,
      'no partial order row persisted after exhausted-retry rollback'
    );
