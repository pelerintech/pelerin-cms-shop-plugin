...
test('migrateDefaultLocale swaps product data between parent tables and translations', async () => {
  ...
  assert.strictEqual(
    prodSimple.name,
    'Carte de programare',
    'Before: product name is in RO (default)'
  );
  ...
  const [prodSimpleAfter] = await db
    .select()
    .from(products)
    .where(eq(products.id, f.simpleProductId));
  assert.strictEqual(
    prodSimpleAfter.name,
    'Programming Book',
    'After: product name is now in EN (new default)'
  );

  // The old 'ro' data should be in translations
  const [roTranslation] = await db
    .select()
    .from(translations)
    .where(
      and(
  ...
  assert.strictEqual(
    roTranslation.name,
    'Carte de programare',
    'After: RO data moved to translations'
  );
});
...
test('migrateDefaultLocale swaps category data', async () => {
  ...
  const [catBooksAfter] = await db
    .select()
    .from(categories)
    .where(eq(categories.id, f.categoryBooksId));
...
test('migrateDefaultLocale swaps attribute data', async () => {
  ...
  const [attrColor] = await db
    .select()
    .from(product_attributes)
    .where(eq(product_attributes.id, f.attrColorId));
...
test('migrateDefaultLocale swaps option value data', async () => {
  ...
  const [optBlack] = await db
    .select()
    .from(product_attribute_options)
    .where(eq(product_attribute_options.id, f.optColorBlackId));
...
test('migrateDefaultLocale is idempotent', async () => {
  ...
  assert.strictEqual(
    prodSimple.name,
    'Programming Book',
    'After two migrations: product name is still in EN'
  );
  ...
  assert.strictEqual(
    roTranslation.name,
    'Carte de programare',
    'After two migrations: RO data still in translations'
  );
});
...
test('migrateDefaultLocale handles new default with no translations', async () => {
  ...
  await db
    .delete(translations)
    .where(and(eq(translations.entity_type, 'product'), eq(translations.locale, 'en')));
  ...
  assert.strictEqual(
    prodSimple.name,
    'Carte de programare',
    'After migration: product name keeps old data when no EN translation exists'
  );
  ...
  assert.strictEqual(
    roTranslation.name,
    'Carte de programare',
    'After migration: RO data moved to translations'
  );
});
