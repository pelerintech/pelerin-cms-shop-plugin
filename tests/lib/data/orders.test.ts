...
import { orders, order_status_history, carts, cart_items, products, product_variants } from '../../../src/db/schema.ts';
...
async function makeCartWithItem(db: any, f: any, cartId = 'cart-o', productId?: string, qty = 2) {
  ...
  await insertFixture(db, 'cart_items', {
    id: 'ci-' + cartId, cart_id: cartId, product_id: productId ?? f.simpleProductId, variant_id: null, quantity: qty,
  });
  return cartId;
}
...
test('createOrder creates an order with status pending, snapshotted items, decrements stock, clears cart', async () => {
  ...
  try {
    ...
    const order = await createOrder(db, {
      ...
      billing_first_name: 'Test', billing_last_name: 'User', billing_address: 'Addr',
      billing_city: 'City', billing_postal_code: '123', billing_country: 'RO',
...
test('getOrderWithItems returns the order with all items', async () => {
  ...
  try {
    ...
    const order = await createOrder(db, {
      order_number: 'ORD-2', user_id: null, customer_type: 'individual', customer_email: 't@e.com',
      customer_name: 'T', customer_phone: null, currency: 'RON', subtotal_net: 10000, vat_total: 500,
      shipping_cost: 0, discount_amount: 0, total: 10500, shipping_type: 'physical',
      billing_first_name: 'T', billing_last_name: 'U', billing_address: 'A', billing_city: 'C',
      billing_postal_code: '1', billing_country: 'RO',       shipping_first_name: 'T', shipping_last_name: 'U', shipping_address: 'A',
      shipping_city: 'C', shipping_postal_code: '1', shipping_country: 'RO',
      shipping_same_as_billing: true, cart_id: cartId,
      items: [{ product_id: f.simpleProductId, variant_id: null, product_name: 'Carte', sku: 'BOOK-001', quantity: 2, price_net: 5000, vat_rate: 0.05, price_gross: 5250, currency: 'RON' }],
    });
...
test('transitionOrderStatus pending→paid updates status and inserts history row', async () => {
  ...
  try {
    ...
    const cartId = await makeCartWithItem(db, f);
    const order = await createOrder(db, {
      order_number: 'ORD-3', user_id: null, customer_type: 'individual', customer_email: 't@e.com',
      customer_name: 'T', customer_phone: null, currency: 'RON', subtotal_net: 10000, vat_total: 500,
      shipping_cost: 0, discount_amount: 0, total: 10500, shipping_type: 'physical',
      billing_first_name: 'T', billing_last_name: 'U', billing_address: 'A', billing_city: 'C',
      billing_postal_code: '1', billing_country: 'RO',       shipping_first_name: 'T', shipping_last_name: 'U', shipping_address: 'A',
    ...
    const history = await db.select().from(order_status_history).where(eq(order_status_history.order_id, order.id));
    assert.ok(history.some(h => h.to_status === 'paid'), 'must have a history row for paid');
  } finally {
    await cleanup();
  }
});
...
test('transitionOrderStatus rejects invalid transition (cancelled→paid)', async () => {
  ...
  try {
    ...
    const cartId = await makeCartWithItem(db, f);
    const order = await createOrder(db, {
      order_number: 'ORD-4', user_id: null, customer_type: 'individual', customer_email: 't@e.com',
      customer_name: 'T', customer_phone: null, currency: 'RON', subtotal_net: 10000, vat_total: 500,
      shipping_cost: 0, discount_amount: 0, total: 10500, shipping_type: 'physical',
      billing_first_name: 'T', billing_last_name: 'U', billing_address: 'A', billing_city: 'C',
      billing_postal_code: '1', billing_country: 'RO',       shipping_first_name: 'T', shipping_last_name: 'U', shipping_address: 'A',
  ...
  try {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      await createOrder(db, {
        order_number: 'ORD-L' + i, user_id: null, customer_type: 'individual', customer_email: 't@e.com',
        customer_name: 'T' + i, customer_phone: null, currency: 'RON', subtotal_net: 5000, vat_total: 250,
    ...
    if (result.orders.length >= 2) {
      assert.ok(result.orders[0].created_at >= result.orders[1].created_at, 'must be DESC by created_at');
    }
  } finally {
    await cleanup();
  }
...
test('listOrders filters by status', async () => {
  ...
  try {
    ...
    const order = await createOrder(db, {
      order_number: 'ORD-F', user_id: null, customer_type: 'individual', customer_email: 't@e.com',
    ...
    assert.ok(result.orders.every(o => o.status === 'cancelled'), 'all returned orders must be cancelled');
  } finally {
    await cleanup();
  }
});
...
test('decrementStock reduces stock for order items (skips null stock)', async () => {
  ...
  try {
    ...
    const order = await createOrder(db, {
      order_number: 'ORD-D', user_id: null, customer_type: 'individual', customer_email: 't@e.com',
      customer_name: 'T', customer_phone: null, currency: 'RON', subtotal_net: 15000, vat_total: 750,
      shipping_cost: 0, discount_amount: 0, total: 15750, shipping_type: 'physical',
      billing_first_name: 'T', billing_last_name: 'U', billing_address: 'A', billing_city: 'C',
      billing_postal_code: '1', billing_country: 'RO',       shipping_first_name: 'T', shipping_last_name: 'U', shipping_address: 'A',
      shipping_city: 'C', shipping_postal_code: '1', shipping_country: 'RO',
      shipping_same_as_billing: true, cart_id: cartId,
      items: [{ product_id: f.simpleProductId, variant_id: null, product_name: 'Carte', sku: 'BOOK-001', quantity: 3, price_net: 5000, vat_rate: 0.05, price_gross: 5250, currency: 'RON' }],
    });
