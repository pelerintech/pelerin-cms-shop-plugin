...
import { product_prices, product_variants, products, product_attribute_assignments, product_attribute_values } from '../../../src/db/schema.ts';
...
async function seedVariantForPriceUpsert(db: any) {
  ...
  await insertFixture(db, 'products', {
    id: productId, sku: 'P', type: 'physical', has_variants: true, vat_rate: 0.19,
    stock: null, category_id: null, active: true, name: 'P', description: '', slug: 'p',
    created_at: NOW8, updated_at: NOW8,
  });
  await insertFixture(db, 'product_variants', { id: variantId, product_id: productId, sku: 'V', stock: 5, active: true });
  // Variant starts with an EUR own price (so the `price_net: null` delete path is exercised).
  await insertFixture(db, 'product_prices', { id: rid8(), product_id: null, variant_id: variantId, currency: 'EUR', price_net: 11 });
...
test('listVariants on a product with dimensions but NO variants returns [] with no error (the currently-failing path)', async () => {
  ...
  try {
    ...
    const { product_variants, product_attribute_values } = await import('../../../src/db/schema.ts');
    const { eq } = await import('drizzle-orm');
    await db.delete(product_attribute_values).where(eq(product_attribute_values.entity_type, 'variant'));
...
test('updateVariant persists prices: upserts RON row and deletes EUR row when price_net is null', async () => {
  ...
  try {
    ...
    const prices = await db.select().from(product_prices).where(eq(product_prices.variant_id, variantId));
...
test('createVariants validates option_ids against dimension attributes FULL option sets (not offered_option_ids)', async () => {
  ...
  try {
    ...
    await db.update(product_attribute_assignments)
    ...
    const rows = await db.select().from(product_attribute_values)
      .where(eq(product_attribute_values.entity_id, created[0].id));
    const colorVal = rows.find(r => r.option_id === f.optColorBlackId);
    assert.ok(colorVal, 'variant dimension value row must be created for the option (full option set used)');
  } finally {
    await cleanup();
  }
});
...
test('listVariants returns effective_prices (own + inherited per currency) alongside own prices', async () => {
  ...
  try {
    ...
    await insertFixture(db, 'product_prices', { id: rid8(), product_id: productId, variant_id: null, currency: 'RON', price_net: 49 });
    await insertFixture(db, 'product_prices', { id: rid8(), product_id: productId, variant_id: null, currency: 'EUR', price_net: 11 });
    // Variant own RON 54 only; EUR inherits from product
    await insertFixture(db, 'product_variants', { id: variantId, product_id: productId, sku: 'V', stock: 5, active: true });
    await insertFixture(db, 'product_prices', { id: rid8(), product_id: null, variant_id: variantId, currency: 'RON', price_net: 54 });
    ...
    const eff = [...v.effective_prices].sort((a: any, b: any) => a.currency.localeCompare(b.currency));
