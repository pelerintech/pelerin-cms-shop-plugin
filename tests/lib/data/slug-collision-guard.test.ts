...
import { createTestDb, seedMinimal, categories, translations as harnessTranslations } from '../../db/harness.ts';
import {
...
async function upsertSlug(db: any, entityType: string, entityId: string, locale: string, slug: string | null, name?: string | null) {
...
test('upsertTranslationWithSlugGuard rejects duplicate slug for different entity', async () => {
  ...
  try {
    ...
    const transRows = await db.select().from(harnessTranslations).where(
      and(
    ...
    assert.equal(transRows.length, 0, 'no translation row should be written for the colliding entity');
  } finally {
    await cleanup();
  }
});
...
test('upsertTranslationWithSlugGuard allows slug change to new unique value', async () => {
  ...
  try {
    ...
    const [trans] = await db.select().from(harnessTranslations).where(
      and(
