...
describe('euPlatesc MAC computation', () => {
  it('produces correct HMAC-MD5 hash from official JS sample', () => {
    ...
    const fields = [
      "1.00", "RON", "123", "Test payment", "testaccount",
      "20181205105150", "522d2594b9d65b850aa01dfc4914ea14",
    ];
    ...
    const macString = "41.003RON312312Test payment11testaccount142018120510515032522d2594b9d65b850aa01dfc4914ea14";
    ...
    assert.strictEqual(result.toLowerCase(), expectedHash.toLowerCase(),
  ...
  it('MAC string is length-prefixed, not raw-joined', () => {
    ...
    assert.notStrictEqual(result.toLowerCase(), rawHash,
  ...
  it('result differs from plain crypto.createHash("md5")', () => {
    const fields = ["1.00", "RON", "123", "Test payment", "testaccount", "20181205105150", "522d2594b9d65b850aa01dfc4914ea14"];
    const key = "00112233445566778899AABBCCDDEEFF";
    const macString = "41.003RON312313Test payment12testaccount142018120510515032522d2594b9d65b850aa01dfc4914ea14";

    const plainMd5 = crypto.createHash('md5').update(macString).digest('hex');
    const result = computeEuplatescHash(fields, key);

    assert.notStrictEqual(result.toLowerCase(), plainMd5,
  ...
  it('null/undefined values contribute dash to MAC string', () => {
    ...
    const binKey = Buffer.from(key, 'hex');
    const expectedHash = crypto.createHmac('md5', binKey).update(expectedMacString, 'utf8').digest('hex');

    assert.strictEqual(resultNull.toLowerCase(), expectedHash,
      'null value must contribute dash to MAC string');
    assert.strictEqual(resultUndefined.toLowerCase(), expectedHash,
      'undefined value must contribute dash to MAC string');
    assert.strictEqual(resultEmpty.toLowerCase(), expectedHash,
  ...
  it('key is hex-decoded to binary, not used as raw string', () => {
    ...
    assert.notStrictEqual(result.toLowerCase(), wrongHash,
