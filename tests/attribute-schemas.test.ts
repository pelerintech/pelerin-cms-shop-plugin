...
describe('Attribute schemas', () => {
  ...
  describe('CreateAttributeSchema', () => {
    it('exists with name, type (enum), and sort_order', () => {
      ...
      assert.ok(content.includes("z.enum(['select', 'text', 'rich_text', 'number', 'boolean'])"), 'type must be enum');
    });
  });
  ...
  describe('UpdateAttributeSchema', () => {
    it('exists as partial of CreateAttributeSchema', () => {
      ...
      assert.ok(content.includes('CreateAttributeSchema.partial()'), 'must be partial of CreateAttributeSchema');
    });
  });

  describe('CreateAttributeOptionSchema', () => {
    it('exists with attribute_id, value, sort_order', () => {
      assert.ok(content.includes('CreateAttributeOptionSchema'), 'CreateAttributeOptionSchema must be defined');
    });
  });

  describe('UpdateAttributeOptionSchema', () => {
    it('exists as partial of CreateAttributeOptionSchema', () => {
      assert.ok(content.includes('UpdateAttributeOptionSchema'), 'UpdateAttributeOptionSchema must be defined');
    });
  });

  describe('CreateAttributeAssignmentSchema', () => {
    it('exists with attribute_id, role enum, offered_option_ids', () => {
      assert.ok(content.includes('CreateAttributeAssignmentSchema'), 'CreateAttributeAssignmentSchema must be defined');
      assert.ok(content.includes("z.enum(['dimension', 'field'])"), 'role must be dimension|field enum');
  ...
  describe('CreateAttributeValueSchema', () => {
    it('exists with entity_type, entity_id, assignment_id, option_id, value_text, value_number, value_boolean', () => {
      assert.ok(content.includes('CreateAttributeValueSchema'), 'CreateAttributeValueSchema must be defined');
      assert.ok(content.includes("z.enum(['product', 'variant'])"), 'entity_type must be product|variant enum');
  ...
  describe('CreateTranslationSchema', () => {
    it('includes new entity types', () => {
      assert.ok(content.includes("'product_attribute'"), 'product_attribute entity_type must be in enum');
      assert.ok(content.includes("'product_attribute_option'"), 'product_attribute_option entity_type must be in enum');
    });
  });
